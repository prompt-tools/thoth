...
Next.js 15 · TypeScript 5.9 · React 19 · Tailwind CSS v3 · Vitest 4。部署在 Vercel：`/api/llm` 服务端代理调用模型（内置 key）；`/api/telemetry` 只接收与当前服务端签名 Journey 状态匹配的无内容结果事件，不保存主题描述、自由文本、回答历史、模型原始输出或最终提示词。
